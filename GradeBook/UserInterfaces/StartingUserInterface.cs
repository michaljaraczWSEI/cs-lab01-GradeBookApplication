...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            if (type == "standard")
            {
                StandardGradeBook gradeBookStandart = new StandardGradeBook(name, isWeight);
                Console.WriteLine("Created gradebook {0}.", name);
                GradeBookUserInterface.CommandLoop(gradeBookStandart);
            }
            else if (type == "ranked")
            {
                RankedGradeBook gradeBookRanked = new RankedGradeBook(name, isWeight);
                Console.WriteLine("Created gradebook {0}.", name);
                GradeBookUserInterface.CommandLoop(gradeBookRanked);
            }
            else 
            {
